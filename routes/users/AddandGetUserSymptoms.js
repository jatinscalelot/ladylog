    ...
    if(userData && userData != null){
      ...
      if(isDateBetween(userData.period_start_date , userData.period_end_date , date)){
        ...
        if(symptoms && symptoms.length > 0){
          ...
          let updateSymptoms = await primary.model(constants.MODELS.usersymptoms , userSymptomsModel).findByIdAndUpdate(symptoms[0]._id , obj , {returnOriginal: false}).lean();
          return responseManager.onSuccess('Symptoms edit successfully...!' , updateSymptoms , res);
        }else{
          let obj = {
            date: date,
            birth_controls: birth_control,
          ...
          const newSysmptoms = await primary.model(constants.MODELS.usersymptoms , userSymptomsModel).create(obj);
          return responseManager.onSuccess('Symptoms add successfully...!' , newSysmptoms , res);
        }
      }else{
