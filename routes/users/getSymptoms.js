...
router.get('/' , helper.authenticateToken , async (req , res) => {
  if(req.token._id && mongoose.Types.ObjectId.isValid(req.token._id)){
    ...
    if(userData && userData != null){
      let symptomData = await primary.model(constants.MODELS.symptoms , symptomModel).find({status: true}).select('_id header_name symptom_name fill_icon unfill_icon').lean();
